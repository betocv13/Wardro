-- AI Outfit Planner Migration (Fixed Version)
...
DO $$
BEGIN
  -- occasion_tags column
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_name = 'clothes' AND column_name = 'occasion_tags'
  ) THEN
    ALTER TABLE clothes ADD COLUMN occasion_tags TEXT[] DEFAULT NULL;
  END IF;

  -- season column
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_name = 'clothes' AND column_name = 'season'
  ) THEN
    ALTER TABLE clothes ADD COLUMN season TEXT CHECK (season IN ('spring', 'summer', 'fall', 'winter', 'all-season')) DEFAULT 'all-season';
  END IF;

  -- wear_count column
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_name = 'clothes' AND column_name = 'wear_count'
  ) THEN
    ALTER TABLE clothes ADD COLUMN wear_count INTEGER DEFAULT 0;
  END IF;

  -- last_worn column
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_name = 'clothes' AND column_name = 'last_worn'
  ) THEN
    ALTER TABLE clothes ADD COLUMN last_worn TIMESTAMPTZ DEFAULT NULL;
  END IF;
END $$;
...
-- ============================================================================
-- 2. Create or update outfits table
-- ============================================================================
...
-- ============================================================================
-- 3. Create or update wear_log table
-- ============================================================================

-- Drop existing table if it exists (to start fresh)
DROP TABLE IF EXISTS wear_log CASCADE;
...
-- Indexes
CREATE INDEX idx_wear_log_user_id ON wear_log(user_id);
CREATE INDEX idx_wear_log_outfit_id ON wear_log(outfit_id);
CREATE INDEX idx_wear_log_worn_on ON wear_log(worn_on DESC);
